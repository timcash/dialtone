...
export class TableSection {
  ...
  async mount() {
    ...
    if (startBtn) {
      startBtn.onclick = async () => {
        const name = prompt(
          "Enter WSL Instance Name:",
          "wsl-node-" + Math.floor(Math.random() * 1000),
        );
        if (!name) return;
  ...
  private renderRows(instances: any[]) {
    ...
    if (!instances || instances.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="padding: 20px; text-align: center; opacity: 0.5;">No active nodes found</td></tr>';
      return;
    }
    ...
    instances.forEach((inst) => {
      if (
        inst.state !== "Running" &&
        inst.state !== "Starting" &&
        this.stoppingNames.has(inst.name)
      ) {
        // Keep it in stopping state until it's actually stopped
...
export function mountTable(container: HTMLElement): VisualizationControl {
  const table = new TableSection(container);
  table.mount();
  return {
    dispose: () => table.dispose(),
    setVisible: (v: boolean) => table.setVisible(v),
  };
}
