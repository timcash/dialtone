package app

import (
	"fmt"
	"os"
	"os/exec"

	"dialtone/dev/plugins/logs/src_v1/go"
)

// RunOpencodeServer starts the opencode AI assistant server
func RunOpencodeServer(port int) {
	opencodePath := os.ExpandEnv("$HOME/.opencode/bin/opencode")
	if _, err := os.Stat(opencodePath); os.IsNotExist(err) {
		logs.Info("opencode binary not found at %s, skipping...", opencodePath)
		return
	}

	logs.Info("Starting opencode server on port %d...", port)
	cmd := exec.Command(opencodePath, "--port", fmt.Sprintf("%d", port))

	// Create log file
	logFile, err := os.OpenFile("opencode.log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0644)
	if err != nil {
		logs.Info("Failed to create opencode log file: %v", err)
		return
	}
	defer logFile.Close()

	cmd.Stdout = logFile
	cmd.Stderr = logFile

	if err := cmd.Run(); err != nil {
		logs.Info("opencode server exited: %v", err)
	}
}
