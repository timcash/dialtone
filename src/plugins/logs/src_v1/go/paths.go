...
func ResolvePaths(start, version string) (Paths, error) {
	...
	return Paths{
		...
		NATSDaemonPID:     filepath.Join(configv1.DefaultDialtoneHome(), "logs", "logs-nats-daemon.pid"),
		NATSDaemonLog:     filepath.Join(configv1.DefaultDialtoneHome(), "logs", "logs-nats-daemon.log"),
