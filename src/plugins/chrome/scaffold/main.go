...
func parseChromeArgs(args []string) (string, []string, error) {
	...
	return "", nil, fmt.Errorf("expected src_v3 as first chrome argument (for example: ./dialtone.sh chrome src_v3 daemon)")
}
...
func printUsage() {
	...
	logs.Info("Versions:")
	logs.Info("  src_v3  - Current managed Chrome service workflow")
}
