...
export function mountLog(container: HTMLElement): VisualizationControl {
  ...
  term.writeln('[DAG LOG] ready');
  term.writeln('[DAG LOG] mode-form enabled: cursor/select/command');
  term.writeln('[DAG LOG] tailing /api/test-log ...');
  ...
  const copySelection = async () => {
    ...
    if (!selected) {
      term.writeln('[DAG LOG] COPY> no selection');
      return;
    }
    try {
      ...
      term.writeln(`[DAG LOG] COPY> ${selected.length} chars`);
    } catch {
      term.writeln('[DAG LOG] COPY> clipboard unavailable');
    }
  };
    ...
    try {
      ...
      if (!res.ok) {
        if (backendState !== 'waiting') {
          term.writeln('[DAG LOG] waiting for backend /api/test-log ...');
          backendState = 'waiting';
        }
        return;
      }
      if (backendState !== 'connected') {
        term.writeln('[DAG LOG] connected to /api/test-log');
