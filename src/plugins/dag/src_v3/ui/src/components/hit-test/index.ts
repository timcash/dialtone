...
class HitTestControl implements VisualizationControl {
  ...
  private setSelectedCube(id: HoveredCubeID) {
    ...
    console.log(`[HitTest #hit-test] touch cube: ${id || 'none'}`);
  }
  ...
  private onClick = (event: MouseEvent) => {
    this.hitTestClientPoint(event.clientX, event.clientY);
  };
  ...
  private attachDebugBridge() {
    (window as Window & {
      dagHitTestDebug?: {
        ...
        clickProjected: (id: Exclude<HoveredCubeID, ''>) => boolean;
      };
    }).dagHitTestDebug = {
      getProjectedPoint: this.getProjectedPoint,
      touchProjected: this.touchProjected,
      clickProjected: (id: Exclude<HoveredCubeID, ''>) => {
        const p = this.getProjectedPoint(id);
  ...
  dispose(): void {
    ...
    if (win.dagHitTestDebug) {
      delete win.dagHitTestDebug;
    }
...
export function mountHitTest(container: HTMLElement): VisualizationControl {
  const canvas = container.querySelector("canvas[aria-label='Hit Test Canvas']") as HTMLCanvasElement | null;
  if (!canvas) throw new Error('hit-test canvas not found');
  canvas.setAttribute('data-wheel-count', '0');
  return new HitTestControl(container, canvas);
}
