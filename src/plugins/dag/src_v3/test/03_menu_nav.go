...
func Run03MenuNavSectionSwitch(ctx *testCtx) (string, error) {
	...
	ctx.logf("LOOKING FOR: Toggle Global Menu")
	if err := ctx.waitAria("Toggle Global Menu", "need menu toggle"); err != nil {
		return "", err
	}
	ctx.logf("LOOKING FOR: DAG Table")
	...
	ctx.logf("LOOKING FOR: Navigate Stage button in menu")
	...
	ctx.logf("LOOKING FOR: Three Canvas after menu nav")
	if err := ctx.waitAria("Three Canvas", "confirm stage visible after nav"); err != nil {
		return "", err
	}
	ctx.logf("LOOKING FOR: Three Canvas data-ready=true after menu nav")
