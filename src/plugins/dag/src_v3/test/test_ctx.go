...
func (t *testCtx) ensureDevServer() error {
	if t.sharedBrowser != nil { // This is a bit of a hack, but if we have a browser we might need a server
		// actually, we should just check if the port is open
	}
	if test_v2.WaitForPort(3000, 100*time.Millisecond) == nil {
		return nil
	}

	t.logf("   [TEST] Starting dev server on port 3000...")
	repoRoot, err := findRepoRoot()
	if err != nil {
		return err
	}

	uiDir := filepath.Join(repoRoot, "src", "plugins", "dag", "src_v3", "ui")
	...
	if _, err := os.Stat(bunBin); err != nil {
		t.logf("   [TEST] Warning: bun not found at %s, falling back to PATH", bunBin)
		bunBin = "bun" // Fallback
	}

	t.logf("   [TEST] Using bun: %s in %s", bunBin, uiDir)
	...
	// Wait for it
	t.logf("   [TEST] Waiting for dev server to be ready on port 3000...")
	if err := test_v2.WaitForPort(3000, 30*time.Second); err != nil {
		return fmt.Errorf("dev server didn't start: %w", err)
	}
	t.logf("   [TEST] Dev server is ready.")

	// We don't have a way to easily kill this yet in teardown if we don't store it
	// but let's just let it run for now or add it to testCtx
	return nil
}
...
func hasAttachableDagDevBrowser() bool {
	...
	for _, p := range procs {
		...
		if p.DebugPort > 0 && chrome_app.HasReachableDevtoolsWebSocket(p.DebugPort) {
			return true
		}
	}
	return false
...
func (t *testCtx) appURL(path string) string {
	...
	return base + urlPath + sep + "test=true" + fragment
}
...
func (t *testCtx) assertMode(expected string) error {
	...
	if mode != expected {
		return fmt.Errorf("form mode mismatch: got=%q expected=%q", mode, expected)
	}
