...
func run(args []string) error {
	...
	subject := fs.String("subject", "mavlink.heartbeat", "Heartbeat subject")
	...
	logs.Info("mavlink_v1 heartbeat publisher started subject=%s nats=%s", *subject, *natsURL)
	for {
		select {
		case <-ctx.Done():
			logs.Info("mavlink_v1 stopping")
			return nil
		case t := <-ticker.C:
			msg := fmt.Sprintf(`{"source":"mavlink_v1","ts":"%s"}`, t.UTC().Format(time.RFC3339Nano))
			if err := nc.Publish(*subject, []byte(msg)); err != nil {
				logs.Warn("mavlink_v1 publish error: %v", err)
				continue
			}
			_ = nc.Flush()
		}
...
func usage() {
	...
	logs.Raw("Commands:")
	logs.Raw("  run [--nats-url URL] [--subject mavlink.heartbeat] [--interval 1s]")
	logs.Raw("  version")
}
