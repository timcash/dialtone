...
func RunMockData(args []string) {
	...
	fmt.Println(">> [MOCK] Starting NATS Server...")
	...
	// 3. Mock HTTP Server (API + MJPEG Stream)
	go func() {
		...
		fmt.Printf("Mock API & Stream Server listening on :%d\n", streamPort)
		if err := http.ListenAndServe(fmt.Sprintf(":%d", streamPort), mux); err != nil {
...
func startMockNATSServer(port, wsPort int) *server.Server {
	...
	go ns.Start()
	ns.SetLoggerV2(logger.GetNATSLogger(), true, true, true)
