...
func RunMockData(args []string) {
	...
	if disableAI {
		fmt.Printf(" - AI Mock: DISABLED (--no-ai)\n")
	}
	...
	// NEW: Start AI Mock Responder (Optional)
	if !disableAI {
		go runMockAIResponder(natsPort)
	}
	...
	go func() {
		...
		streamMux.HandleFunc("/api/init", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Printf("Mock API and Camera Stream listening on :%d\n", streamPort)
...
func startMockNATSServer(port, wsPort int) *server.Server {
	...
	fmt.Printf("NATS WebSocket Config: Host=%s, Port=%d, SameOrigin=%v, AllowedOrigins=%v\n",
		opts.Websocket.Host, opts.Websocket.Port, opts.Websocket.SameOrigin, opts.Websocket.AllowedOrigins)
	...
	ns.SetLogger(logger.GetNATSLogger(), true, true)
...
func runMockAIResponder(natsPort int) {
	nc, err := nats.Connect(fmt.Sprintf("nats://127.0.0.1:%d", natsPort))
	if err != nil {
		fmt.Printf("AI Mock failed to connect to NATS: %v\n", err)
		return
	}
	// defer nc.Close() // Keep open

	fmt.Println("AI Mock Responder Listening on ai.opencode.input...")

	nc.Subscribe("ai.opencode.input", func(m *nats.Msg) {
		input := string(m.Data)
		fmt.Printf("[Mock AI] Received input: %s\n", input)

		// Simulate "thinking" delay
		time.Sleep(500 * time.Millisecond)

		// Respond
		response := fmt.Sprintf("Mock AI Response to: %s", input)
		nc.Publish("ai.opencode.output", []byte(response))
	})
}
