...
import { getUISharedTemplate, renderUISharedTemplate, type UISharedTemplateID } from '../../../../ui/templates';
...
function mountSphereScene(canvas: HTMLCanvasElement): { dispose: () => void; setVisible: (visible: boolean) => void } {
  ...
  const ground = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x0d1522, roughness: 0.9, metalness: 0.02 })
  );
...
function applyMode(sectionId: UISharedTemplateID, mode: 'fullscreen' | 'calculator'): void {
...
function toggleModeFor(sectionId: UISharedTemplateID): void {
  const current = sectionModes.get(sectionId) ?? getUISharedTemplate(sectionId).defaultMode;
...
function bindInteractions(sectionId: UISharedTemplateID, container: HTMLElement): void {
  if (container.dataset.bound === '1') return;
  container.dataset.bound = '1';

  const modeButton = Array.from(container.querySelectorAll('button')).find((b) => b.textContent?.trim().toLowerCase() === 'mode');
  if (modeButton) {
    modeButton.addEventListener('click', () => toggleModeFor(sectionId));
  }

  if (sectionId === 'table') {
    const refresh = container.querySelector("button[data-action='table-refresh']") as HTMLButtonElement | null;
    const status = container.querySelector("dd[data-field='table-status']") as HTMLElement | null;
    if (refresh && status) {
  ...
  if (sectionId === 'three-calculator') {
    const add = container.querySelector("button[data-action='three-add']") as HTMLButtonElement | null;
    if (add) {
  ...
  if (sectionId === 'terminal') {
    const send = container.querySelector("button[data-action='terminal-send']") as HTMLButtonElement | null;
    const input = container.querySelector('input') as HTMLInputElement | null;
    const pre = container.querySelector('pre') as HTMLElement | null;
    const status = container.querySelector("dd[data-field='terminal-status']") as HTMLElement | null;
    const run = () => {
...
for (const sectionId of sectionIDs) {
  ...
  menu.addButton(template.title, `Navigate ${template.title}`, () => sections.navigateTo(sectionId));
}
...
const initial = sectionIDs.includes(initialRaw as UISharedTemplateID) ? (initialRaw as UISharedTemplateID) : 'hero';
