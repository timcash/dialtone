	...
	for _, section := range sections {
		...
		err := chromedp.Run(sectCtx,
			...
			chromedp.ActionFunc(func(ctx context.Context) error {
				...
				switch section {
				case "s-home":
					expected = "ORBITAL DYNAMICS"
				case "s-about":
					expected = "VISION GRID PRESETS"
				case "s-robot":
					expected = "KINEMATIC SOLVER"
				case "s-neural":
					expected = "NEURAL TOPOLOGY"
				case "s-math":
					expected = "MANIFOLD PROJECTIONS"
				case "s-cad":
					expected = "PARAMETRIC GEAR"
				case "s-policy":
					expected = "MARKOV SCENARIOS"
				case "s-music":
