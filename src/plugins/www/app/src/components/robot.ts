...
class Joint {
    ...
    private applyRotation(): void {
        ...
        switch (this.axis) {
            case 'x': this.group.rotation.x = this.angle; break;
            case 'y': this.group.rotation.y = this.angle; break;
            case 'z': this.group.rotation.z = this.angle; break;
        }
    }
}
...
class RobotArmVisualization {
    ...
    initConfigPanel() {
        const panel = document.getElementById('robot-config-panel') as HTMLDivElement | null;
        const toggle = document.getElementById('robot-config-toggle') as HTMLButtonElement | null;
        ...
        const addCheckbox = (label: string, checked: boolean, onChange: (v: boolean) => void) => {
            const row = document.createElement('div');
            row.className = 'earth-config-row';
        ...
        addHeader('IK Mode');
        addCheckbox('Auto Track Target', this.autoAnimate, (v) => { this.autoAnimate = v; });
        addButton('New Target', () => this.pickNewTarget());

        addHeader('Camera');
        addSlider('Distance', this.cameraRadius, 6, 20, 0.5, (v) => { this.cameraRadius = v; }, (v) => v.toFixed(1));
        addSlider('Height', this.cameraHeight, 1, 12, 0.5, (v) => { this.cameraHeight = v; }, (v) => v.toFixed(1));
        addSlider('Orbit Speed', this.cameraOrbitSpeed, 0, 0.5, 0.01, (v) => { this.cameraOrbitSpeed = v; }, (v) => v.toFixed(2));
