...
class CurriculumVisualization {
    ...
    initInnerOrbit() {
        ...
        this.torusWireframe = new THREE.LineSegments(new THREE.WireframeGeometry(wireGeo), wireMat);
        ...
        const cubeMat = new THREE.LineBasicMaterial({ color: 0x8b5cf6, transparent: true, opacity: 0.8 });
        this.cube = new THREE.LineSegments(new THREE.WireframeGeometry(cubeGeo), cubeMat);
    ...
    initMiddleOrbit() {
        ...
        const icoMat = new THREE.LineBasicMaterial({ color: 0x10b981, transparent: true, opacity: 0.7 });
        this.icosahedron = new THREE.LineSegments(new THREE.WireframeGeometry(icoGeo), icoMat);
    ...
    initOuterOrbit() {
        ...
        const mobiusCurve = new THREE.CatmullRomCurve3(mobiusPoints.filter((_, i) => i % 5 === 0), true);
        ...
        const dodecaMat = new THREE.LineBasicMaterial({ color: 0xf97316, transparent: true, opacity: 0.7 });
        this.dodecahedron = new THREE.LineSegments(new THREE.WireframeGeometry(dodecaGeo), dodecaMat);
        ...
        const cylMat = new THREE.LineBasicMaterial({ color: 0x3b82f6, transparent: true, opacity: 0.7 });
        this.cylinder = new THREE.LineSegments(new THREE.WireframeGeometry(cylGeo), cylMat);
        ...
        const kleinCurve = new THREE.CatmullRomCurve3(kleinPoints.filter((_, i) => i % 7 === 0), true);
    ...
    initGraph() {
        ...
        const edges = [
            [0, 1], [1, 2], [2, 3], [0, 4], [0, 5], [1, 5], [1, 6], [2, 6], [2, 7], [3, 7], [3, 8],
            [4, 5], [5, 6], [6, 7], [7, 8], [4, 9], [5, 9], [5, 10], [6, 10], [7, 10], [7, 11], [8, 11],
            [9, 10], [10, 11],
        ];
    ...
    initMappingLines() {
        const createCurve = (start: THREE.Vector3, end: THREE.Vector3, color: number) => {
        ...
        this.scene.add(createCurve(graphPos, new THREE.Vector3(-4, 0, 0), 0xec4899));
        this.scene.add(createCurve(graphPos, new THREE.Vector3(0, 0, 4), 0x3b82f6));
        this.scene.add(createCurve(graphPos, new THREE.Vector3(0, 0, -4), 0x8b5cf6));
    }

    initConfigPanel() {
        const panel = document.getElementById('curriculum-config-panel') as HTMLDivElement | null;
        const toggle = document.getElementById('curriculum-config-toggle') as HTMLButtonElement | null;
        ...
        addSection('Camera');
        addSlider('Radius', this.cameraOrbitRadius, 8, 35, 0.5, (v) => { this.cameraOrbitRadius = v; });
        addSlider('Height', this.cameraHeight, -5, 15, 0.5, (v) => { this.cameraHeight = v; });
        addSlider('Height Osc', this.cameraHeightOsc, 0, 6, 0.1, (v) => { this.cameraHeightOsc = v; });
        addSlider('Height Speed', this.cameraHeightSpeed, 0, 0.5, 0.01, (v) => { this.cameraHeightSpeed = v; });
        addSlider('Orbit Speed', this.cameraOrbitSpeed, 0, 0.2, 0.005, (v) => { this.cameraOrbitSpeed = v; }, (v) => v.toFixed(3));
        addSlider('Look At Y', this.cameraLookAtY, -5, 10, 0.5, (v) => { this.cameraLookAtY = v; });

        addSection('Orbit Rotation');
        addSlider('Inner Speed', this.innerOrbitSpeed, 0, 0.01, 0.0005, (v) => { this.innerOrbitSpeed = v; }, (v) => v.toFixed(4));
        addSlider('Middle Speed', this.middleOrbitSpeed, 0, 0.01, 0.0005, (v) => { this.middleOrbitSpeed = v; }, (v) => v.toFixed(4));
        addSlider('Outer Speed', this.outerOrbitSpeed, 0, 0.01, 0.0005, (v) => { this.outerOrbitSpeed = v; }, (v) => v.toFixed(4));
    ...
    animate = () => {
        ...
        this.nodes.forEach((node, i) => {
            ...
            mat.uniforms.uIntensity.value = 1.2 + Math.sin(this.time * 2 + i * 0.7) * 0.5;
        });
