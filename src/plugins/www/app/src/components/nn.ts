...
class NeuralNetworkVisualization {
    ...
    createConnectionCurve(from: THREE.Vector3, to: THREE.Vector3): THREE.QuadraticBezierCurve3 {
    ...
    initConfigPanel() {
        const panel = document.getElementById('nn-config-panel') as HTMLDivElement | null;
        const toggle = document.getElementById('nn-config-toggle') as HTMLButtonElement | null;
        ...
        addSection('Camera');
        addSlider('Radius', this.cameraRadius, 5, 30, 0.5, (v) => { this.cameraRadius = v; });
        addSlider('Height', this.cameraHeight, -5, 10, 0.5, (v) => { this.cameraHeight = v; });
        addSlider('Height Osc', this.cameraHeightOsc, 0, 5, 0.1, (v) => { this.cameraHeightOsc = v; });
        ...
        addSlider('Look At Y', this.cameraLookAtY, -5, 5, 0.5, (v) => { this.cameraLookAtY = v; });

        addSection('Signal');
        addSlider('Signal Speed', this.signalSpeed, 0.1, 2, 0.1, (v) => { this.signalSpeed = v; });
    ...
    animate = () => {
        ...
        this.neurons.forEach((neuron) => {
            ...
            const wave = Math.sin(this.signalTime * 3 - layerPhase + neuron.index * 0.2);
        ...
        this.connections.forEach(conn => {
            ...
            const phase = this.signalTime * 2 - conn.from.layer * 0.6 + conn.pulseOffset;
