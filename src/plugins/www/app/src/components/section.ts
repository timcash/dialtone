...
export class SectionManager {
    ...
    constructor(options?: { debug?: boolean }) {
        ...
        if (this.debug) {
            console.log('%c🌐 SectionManager initialized', 'color: #22c55e; font-weight: bold; font-size: 14px');
            console.log('%c   Lazy loading: enabled | Animations pause when off-screen', 'color: #888');
        }
    }
    ...
    observe(threshold = 0.1): void {
                ...
                if (entry.isIntersecting) {
                    this.load(sectionId).then(() => {
                        ...
                        if (control) {
                            ...
                            if (this.debug) {
                                console.log(`%c[${sectionId}] ▶️ AWAKE - animation running`, 'color: #3b82f6; font-weight: bold');
                            }
                        }
                    });
                } else {
                    ...
                    if (control) {
                        ...
                        if (this.debug) {
                            console.log(`%c[${sectionId}] 💤 SLEEP - animation paused`, 'color: #8b5cf6');
                        }
                    }
                }
            });
        ...
        if (this.debug) {
            console.log(`%c[SectionManager] 👀 Observing ${observedSections.length} sections:`, 'color: #06b6d4', observedSections.join(', '));
        }
    }
    ...
    eagerLoad(sectionId: string): Promise<void> {
        if (this.debug) {
            console.log('%c[SectionManager] 🚀 Eagerly loading first section...', 'color: #f59e0b');
        }
    ...
    async load(sectionId: string): Promise<void> {
        ...
        if (this.debug) {
            console.log(`%c[${sectionId}] 📦 Starting lazy load...`, 'color: #f59e0b; font-weight: bold');
        }
        ...
        const loadPromise = config.load()
            .then(control => {
                ...
                if (this.debug) {
                    ...
                    console.log(`%c[${sectionId}] ✅ Loaded and mounted (${elapsed}ms)`, 'color: #22c55e; font-weight: bold');
                }
            })
            .catch(err => {
                console.error(`%c[${sectionId}] ❌ Failed to load:`, 'color: #ef4444', err);
            })
            .finally(() => {
