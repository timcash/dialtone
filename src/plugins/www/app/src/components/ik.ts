...
export class IKSolver {
    ...
    private adjustJoint(jointIndex: number, target: THREE.Vector3): void {
        ...
        parentWorldMatrix.decompose(new THREE.Vector3(), parentQuat, new THREE.Vector3());
        ...
        const toEndOnPlane = toEnd.clone().sub(
            axisWorld.clone().multiplyScalar(toEnd.dot(axisWorld))
        );
        const toTargetOnPlane = toTarget.clone().sub(
            axisWorld.clone().multiplyScalar(toTarget.dot(axisWorld))
        );
        ...
        const cross = new THREE.Vector3().crossVectors(toEndOnPlane, toTargetOnPlane);
