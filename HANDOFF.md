...
- Prefer task rooms and task ids in tests and operator commands.
- Use `./dialtone.sh <plugin> <src_vN> format` after code edits to plugin code.
- Use focused tests one by one while migrating.
- Do not add new public `subtone-*` commands, room names, or transcript text.
...
If a file needs to be copied explicitly into `/home/user/dialtone` before rerunning a visible test, use a tmux-visible command like:

```powershell
.\wsl-tmux.cmd "cp /mnt/c/Users/timca/dialtone/src/plugins/repl/src_v3/go/repl/core_runtime.go /home/user/dialtone/src/plugins/repl/src_v3/go/repl/core_runtime.go"
```
...
- normal routed command lifecycle is task-first:
  - `Request received.`
  - `Task queued as task-...`
  - `Task room: task.task-...`
  - `Task log: .../task-....log`
...
Recent tmux-visible focused tests that passed:
...
1. finish converting remaining tests and helpers that still refer to old process-first names or assumptions
2. tighten `task list`, `task show`, `task log`, and `task kill` around NATS-backed task snapshots
3. make service desired and observed state more explicit for `service list` and `service show`
4. add or finish the simple REPL test daemon fixture from the test plan
...
7. use `legion` for long-lived `chrome src_v3` service reconciliation
...
- When a focused test fails, inspect the task room, task log, and operator commands before making a broad code change.
