...
## Goal
Implement the autonomous developer loop and subagent integration in `dialtone-dev` as described in issue #34 and the design document.
...
- [ ] test_issue_ranking: Verify the developer loop can fetch and rank issues by capability labels.
- [ ] test_feature_dir_setup: Verify subagent task files are correctly created in `features/` directory.
- [ ] test_full_loop_e2e: Run a mock end-to-end cycle from issue selection to PR creation.
